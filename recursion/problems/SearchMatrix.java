public class SearchMatrix {
  ...
  public boolean searchMatrix(int[][] matrix, int target) {
    boolean ans = solve(0, matrix[0].length - 1, matrix, target);
    return ans;
  }

  private boolean solve(int r, int c, int[][] matrix, int target) {
    ...
    // either move left or down
    // choose wisely!

    if (curr > target) {
      ans = solve(r, c - 1, matrix, target);
    }
    else {
      ans = solve(r + 1, c, matrix, target);
    }
